...
class CardAnchor {
    ...
    public static Card[] mCardRecent = new Card[MAX_CARDS];
...
//        break;
    ...
    public CardAnchor() {
        ...
        for (int i = 0; i < mCardRecent.length; i++) {
            if (mCardRecent[i] == null)
                mCardRecent[i] = new Card(0, 0, 0);
        }
        mCardCount = 0;
        mHiddenCount = 0;
        mLeftEdge = -1;
    ...
    public void AddMoveCard(MoveCard moveCard) {
        ...
        int flag = 0;
        for (int i = 0; i < mCardRecent.length; i++) {
            if (mCardRecent[i].GetValue() == ca.GetValue()) {
                flag = 1;
            }
        }

        for (int i = 0; i < mCardRecent.length; i++) {
            if (mCardRecent[i].GetValue() < 1) {
                mCardRecent[i] = new Card(ca.GetValue(), ca.GetSuit(), ca.getmTag());
                break;
            }
        }
        if (flag == 0) {
